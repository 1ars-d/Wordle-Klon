...
import { INIT_END_MESSAGE_STATE, INIT_STATE, TARGET_WORDS } from "./Data/DATA";
...
function App() {
         ...
         if (tiles.errors.length > 0) {
            for (let i = 0; i < tiles.errors.length; i++) {
               if (Date.now() > tiles.errors[i].create + 500) {
                  dispatchTiles({ type: "REMOVE_ERROR_SHOW", index: i });
                  setTimeout(() => {
                     dispatchTiles({ type: "REMOVE_ERROR", index: i });
                  }, 200);
               }
            }
         }
   ...
   const keyAddHandler = (key) => {
      switch (key) {
         case "enter":
            dispatchTiles({ type: "ENTER", target: target });
            break;
         case "backspace":
            dispatchTiles({ type: "DELETE" });
            break;
         default:
            if (key.toLowerCase().match(/[a-z]/i)) {
               dispatchTiles({ type: "ADD_KEY", key: key });
            }
      }
   };
   ...
   const removeWiggle = () => {
      dispatchTiles({ type: "REMOVE_WIGGLE" });
   };
   return (
